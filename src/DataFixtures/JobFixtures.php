...
class JobFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        $jobExtremeSensio->setCategories($manager->merge($this->getReference('category-design')));
        ...
        $jobExpired = new Jobs();
        $jobExpired->setCategories($manager->merge($this->getReference('category-programming')));
        ...
        for ($i = 100; $i <= 130; $i++) {
            $job = new Jobs();
            $job->setCategories($manager->merge($this->getReference('category-programming')));
